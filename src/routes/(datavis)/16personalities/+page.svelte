<script lang="ts">
  ...
  const labelPosition = (angle: number, r: number) =>
    d3.pointRadial((angle / 100) * (Math.PI * 2), r);
  ...
  const highlight = (grp: string, enable: boolean = true) => {
    return (e: Event) => {
      ...
      document
        ...
        .forEach((e) =>
          enable ? e.classList.add("selected") : e.classList.remove("selected"),
        );
    };
  };
</script>
...
<section>
  <div class="page">
    ...
    >
      <defs>
        ...
        >
          <path
            d="M0,1 H1000"
            style="stroke:var(--foreground); stroke-width:1"
          />
        </pattern>
        <pattern
          id="N"
          ...
          <g class="personality">
            ...
            <text
              ...
              transform="translate({labelPosition(
                (starts[key] + starts[key] + measures[key]) / 2,
                220,
              )})"
            >
              {key}
      ...
      <g class="legend" transform="translate(0,500)">
        {#each ["I", "N", "F", "P", "E", "S", "T", "J"] as key, index}
          <g
            transform="translate({112.5 + (index % 4) * 75},{Math.floor(
              index / 4,
            ) * 50})"
            onmouseover={highlight(key)}
            onfocus={highlight(key)}
    ...
    <div class="notes">
      Data Source: <a
        href="https://personalitymax.com/personality-types/population-gender/"
        target="_blank"
      >
        personalitymax.com
      </a>
