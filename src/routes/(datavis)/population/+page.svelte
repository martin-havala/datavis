<script lang="ts">
  ...
  import type {
    Population,
    PopulationPosition,
  } from "$lib/assets/population/population.model";
  ...
  let highlight: PopulationPosition | null = $state(null);

  onMount(() => {
    svg = d3.select(svgElem).attr("viewBox", `0 0 ${width},${height}`);

    d3.json(populationUrl).then((data) => {
      const dataMap: { [id: string]: Population } = (
        data as Population[]
      ).reduce((a, c) => ({ ...a, [c.ISO3_code]: c }), {});
      const popPosData = tileMap
        ...
        .sort((a, b) =>
          a.coordinates && b.coordinates && a.coordinates[1] > b.coordinates[1]
            ? 1
            : -1,
        );
      ...
      const grp = mapSelection
        ...
        .attr("class", (d) => `cntry map ${d["region"]} ${d.subRegion}`)
        ...
        .attr("d", (d) =>
          d3
            ...
            ...d.PopTotal_Medium.map((p, i) => [
              ...
              innerTileSizeY * 0.5 -
                innerTileSizeY * 0.5 * (p / d.PopTotal_2020 - 1),
            ]),
          ] as [number, number][]),
        )
...
<section>
  <div class="page">
    ...
    <div class="notes">
      ...
      <span
        >Map source: <a href="https://github.com/mustafasaifee42/Tile-Grid-Map"
          >Tile Grid Map</a
        ></span
      >
    </div>
