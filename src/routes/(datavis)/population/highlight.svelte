<script lang="ts">
  ...
  interface Props {
    highlight: PopulationPosition;
  }

  let { highlight = $bindable() }: Props = $props();
  const factor = 0.1;
  let ratio = $derived(
    highlight?.PopTotal_Medium[year(2100)] / highlight?.PopTotal_2020,
  );
  let density = $derived(Math.round(highlight?.PopDensity_Medium[year(2100)]));
  let density20 = $derived(Math.round(highlight?.PopDensity_2020));
  let mfRatio = $derived(
    (
      highlight?.PopMale_Medium[year(2100)] /
      highlight?.PopFemale_Medium[year(2100)]
    ).toFixed(3),
  );
  let mfRatio20 = $derived(
    (highlight?.PopMale_2020 / highlight?.PopFemale_2020).toFixed(3),
  );
  let mediumLine = $derived(
    d3
      .line()
      .x((d) => d[0])
      .y((d) => d[1])([
      ...
      ...highlight?.PopTotal_Medium.map((p, i) => [
        ((i + 1) / 8) * chartWidth,
        chartHeightCenter -
          chartHeight * factor * (p / highlight?.PopTotal_2020 - 1),
      ]),
    ] as [number, number][]),
  );
  ...
  let positions = $derived({
    ...
    low:
      ...
      chartHeight *
        factor *
        (highlight.PopTotal_Low[year(2100)] / highlight?.PopTotal_2020 - 1),
    mediumValue: d3.format(".2s")(highlight.PopTotal_Medium[year(2100)] * 1000),
    medium:
      chartHeightCenter -
      chartHeight *
        factor *
        (highlight.PopTotal_Medium[year(2100)] / highlight?.PopTotal_2020 - 1),
    highValue: d3.format(".2s")(highlight.PopTotal_High[year(2100)] * 1000),
    high:
      chartHeightCenter * 0.99 -
      chartHeight *
        factor *
        (highlight.PopTotal_High[year(2100)] / highlight?.PopTotal_2020 - 1),
  });
</script>
...
>
  <g class="cntry {highlight.region} {highlight.subRegion}">
    ...
    <path
      d="M{chartWidth},{chartHeightCenter},H0 "
      style="stroke:white; stroke-dasharray:2 2;"
    />
    <path
      d="M0,{chartHeightCenter + 7},v-7 M{chartWidth},{chartHeightCenter +
        7},v-7 "
      style="stroke:white;"
    />
    ...
    <g transform="translate({chartWidth + 10},0)">
      <text class="position" y={positions.high}
        >{positions.highValue} (high)</text
      >
      <text class="position" y={positions.medium}
        >{positions.mediumValue} (medium)</text
      >
    ...
    {#if highlight}
      <g transform="translate(0,{chartHeightCenter + 40})">
        <rect
          class="bgr {highlight.region} {highlight.subRegion}"
          width="20"
          height="20"
          x="10"
          y="0"
        ...
        <text class="changeValue" x="35" y="30">
          Population change: {Math.sign(ratio - 1) > 0 ? "+" : ""}{Math.round(
            100 * ratio - 100,
          )}%
        </text>
...
<style lang="scss">
  ...
  text {
    ...
    dominant-baseline: baseline !important;
  }
  .rangeStroke {
    stroke: white;
    stroke-opacity: 0.6;
