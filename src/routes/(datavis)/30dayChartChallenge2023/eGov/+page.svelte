<script lang="ts">
  ...
  onMount(() => {
    ...
    d3.csv(eGovURL, d3.autoType).then((resData) => {
      ...
      world["GT1"] = data.map((d) => d["GT1"]).sort()[
        Math.floor(data.length / 2)
      ];
      world["Level"] = data.map((d) => d["Level"]).sort()[
        Math.floor(data.length / 2)
      ];
      data.push(world);
      data.forEach((d) => {
        [d.begin, d.end] = d3.extent(
          EGOV_COLS.map((key) => +d[key]).filter(Boolean),
        ) as [number, number];
        const reduced = EGOV_COLS.reduce(
          (a, c) => {
            const year = +d[c];
            if (a[year]) {
              a[year].push(c);
      ...
      data.sort((a, b) =>
        a["Code"] == "World median" || a["GTMI"] > b["GTMI"] ? -1 : 1,
      );
      svgSelection = d3
        .select(svgElem)
      ...
      const countries = svgSelection
        ...
        .attr(
          "class",
          (d) => `country ${d["Code"]} ${d["Level"]} maturityGroup${d["Grp"]}`,
        )
        ...
        .on("click", (e, d) => {
          selectedCountry &&
            document
              .querySelector(`g.${selectedCountry.Code}`)
      ...
      countries
        ...
        .attr("d", (d) =>
          arc(
            (d.index / d.count) * 2 * Math.PI,
            ((1 + d.index) / d.count) * 2 * Math.PI,
          ),
        )
        .style(
          "fill",
          (d) => COLOR_SCALE[EGOV_COLS.findIndex((c) => c == d.value)],
        );
